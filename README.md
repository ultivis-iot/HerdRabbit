...
| `HERDR_WEB_ROLE` | `hub` | `hub` serves people; `leaf` serves only its hub and answers nothing else. A leaf must bind loopback, must name at least one peer, and must have no password -- the process refuses to start otherwise. |
...
| `HERDR_WEB_PEER_ADDRESSES` | empty | Tailnet addresses a leaf accepts, comma-separated. A login proves the account, not the device, so without this a leaf answers every device that account owns. |
...
- A **leaf** is protected by the identity headers Tailscale Serve adds, not by a password. That only holds while Serve is the one thing that can reach its socket, so a leaf refuses to start unless it binds loopback, and refuses each request that arrives any other way. The consequence worth naming: **any local process on a leaf, as any user, can forge those headers and reach its whole API.** For the service account this changes nothing -- it already has the `herdr` CLI -- but for other users on that machine it is a real widening compared with a leaf that had a password.
