...
- `Esc`, `Tab`, `Shift+Tab`, 방향키 및 `Enter` 전송
- 프로젝트 이름 변경
...
워크스페이스·탭·패인 생성이나 삭제, Herdr 영구 세션명 변경, OS 푸시 알림은 현재 제공하지 않습니다.
...
HerdrBridge Node.js 서버
        ...
        │ execFile 인자 배열
        ▼
로컬 herdr CLI ── 기존 Herdr 워크스페이스와 패인
```

HerdrBridge는 자체 터미널 세션을 만들지 않습니다. 화면과 입력은 현재 실행 중인 Herdr 서버의 기존 패인에 연결됩니다.
...
- 프로젝트명 오른쪽의 연필 버튼을 누르면 이름을 편집합니다.
...
테스트는 가짜 Herdr 실행 파일과 임시 상태를 사용하므로 실제 Herdr 패인에 입력하거나 프로젝트명을 변경하지 않습니다.
...
- 프로젝트 ID, 패인 ID, 이름, 텍스트 길이, 출력 행 수 및 명령 실행 시간을 제한합니다.
