...
서버는 전부 또 다른 HerdRabbit입니다. 다이얼로그가 tailnet의 머신 목록을 가져와 각각이 무엇인지 알려줍니다 — 추가 가능, 이미 추가됨, HerdRabbit은 있지만 이 머신을 안 받아줌, 버전 불일치, 아무것도 없음. 추가 가능한 것을 고르면 이름과 주소가 채워집니다. 기본 포트가 아닌 머신이나 tailnet을 읽을 수 없는 경우에는 주소를 직접 입력합니다.
