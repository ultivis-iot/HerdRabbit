...
- `scrypt` 비밀번호 해시, HMAC 서명 세션, `HttpOnly` 쿠키, 창 단위 실행 토큰 및 CSRF/동일 출처 검증
- Node.js 내장 `node:test`, 가짜 Herdr 실행 파일을 사용한 단위·통합 테스트

실시간 연결에 WebSocket을 사용하지 않습니다. 터미널 출력은 1초마다, 세션 상태는 2초마다 HTTP polling으로 갱신합니다.
...
- 프로젝트명·탭 이름·마지막 작업 요청을 포함하는 상태별 PWA 알림
- 데스크톱 접이식 사이드바와 모바일 오버레이 탐색기
- 라이트·다크 모드
- 터미널 출력과 입력창의 글자 크기 조절 및 설정 저장
- 데스크톱과 모바일에서 설치 가능한 PWA
- 선택형 인스턴스 비밀번호, 7일 고정 만료 및 PWA를 새로 열 때 재로그인
...
- 같은 상태 이벤트는 한 번만 알리고, 같은 세션의 이전 알림은 최신 상태로 교체합니다. 알림을 누르면 해당 세션을 엽니다.
...
작업 요청, 프로젝트명과 탭 이름이 잠금 화면 알림에 표시될 수 있으므로 개인 기기에서만 알림을 허용하세요.
