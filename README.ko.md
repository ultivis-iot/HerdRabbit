...
**HerdRabbit on that machine**이 기본입니다. 상대 머신이 leaf 모드로 설치된 자기 HerdRabbit을 돌리고, 이쪽이 tailnet으로 그걸 호출합니다. `http://100.101.171.95:38787` 같은 주소만 입력하면 됩니다. 여기에 저장되는 것은 이름과 주소뿐입니다 — leaf는 요청이 도착한 주소로 자기 허브를 알아보므로 보관할 키나 비밀번호가 없습니다. 양쪽 HerdRabbit 버전이 같아야 하며, 다르면 그 서버가 양쪽 버전과 함께 offline으로 표시됩니다. 링크된 서버의 파일 탐색과 프로젝트 생성은 아직 지원하지 않습니다.
