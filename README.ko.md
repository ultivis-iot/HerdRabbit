...
출력은 revision 기반 HTTP delta polling으로 갱신합니다. 기본 주기는 1초이며, 모바일 데이터·데이터 절약 모드에서 작업 중이면 5초입니다. 입력 전송 직후에는 즉시 조회하고 250ms 후 재조회하며 잠시 300ms 주기로 갱신합니다. 화면에서 보고 있는 세션의 알림은 생략하고, 다른 세션과 백그라운드 상태에서는 알림을 유지합니다.
