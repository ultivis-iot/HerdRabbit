...
대화상자에는 CLI마다 지금 쓰는 계정과 저장된 계정이 나오고, 플랜이 함께 보입니다. Claude Code는 다시 로그인해야 하는 날짜도 보이며, 로그인한 뒤 대략 한 달입니다. Codex는 그 날짜를 기록하지 않아 표시하지 않습니다. 계정에 관해 그 밖의 정보는 보여주지 않습니다.

- **Save**는 지금 로그인된 계정을 저장합니다.
- **Add account**는 `AI login: …` 프로젝트를 열고 그곳에 로그인 명령을 입력합니다. `claude auth login` 또는 `codex login --device-auth`이며, 각자 비어 있는 전용 디렉터리를 가리킵니다. 지금 쓰는 계정이 있는 곳에서 로그인하면 그 계정의 토큰이 폐기되기 때문에 따로 떨어진 곳에서 로그인합니다. Codex의 device code 로그인은 ChatGPT 보안 설정에서 먼저 켜야 합니다. 터미널에 로그인됐다고 나오면 AI accounts를 다시 열고 **Finish**를 누르세요. 프로젝트는 닫힙니다. 그 프로젝트에 입력한 내용은 입력 기록에 남지 않습니다.
...
- **Remove**는 저장된 사본을 지웁니다. 어디에서도 계정을 로그아웃시키지는 않습니다.
