...
서버는 전부 또 다른 HerdRabbit입니다. 다이얼로그가 tailnet의 머신 목록을 가져와 각각이 무엇인지 알려줍니다 — 추가 가능, 이미 추가됨, HerdRabbit은 있지만 이 머신을 안 받아줌, 버전 불일치. 추가 가능한 것을 고르면 이름과 주소가 채워집니다. 손댈 게 없는 머신(HerdRabbit 없음, 오프라인)은 개수만 남기고 접힙니다 — tailnet이 커도 목록이 읽히고, 기대했던 머신이 안 보일 때 펼쳐서 이유를 확인할 수 있습니다. 머신은 고르는 것이지 입력하는 것이 아닙니다. 주소는 tailnet에서 오고, 정할 것은 여기서 뭐라고 부를지뿐입니다. 그래서 기본 포트가 아닌 머신이나 tailnet을 읽을 수 없는 경우에는 이 다이얼로그로 추가할 수 없습니다.
