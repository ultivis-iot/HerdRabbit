...
로컬 Herdr는 기본으로 유지합니다. 사이드바 상단의 **+ → Connect Server**에서 서버를 추가하면, 등록한 모든 서버가 **서버 → Herdr 세션 → 프로젝트 → 탭/패인**으로 함께 표시됩니다.

서버에 닿는 방법은 **Connection**에서 두 가지 중 고릅니다.

**HerdRabbit on that machine**이 기본입니다. 상대 머신이 leaf 모드로 설치된 자기 HerdRabbit을 돌리고, 이쪽이 tailnet으로 그걸 호출합니다. `https://dev-box.tailnet.ts.net:38787` 같은 주소만 입력하면 됩니다. 여기에 저장되는 것은 이름과 주소뿐입니다 — leaf가 Tailscale Serve가 찍어주는 신원 헤더로 호출자를 확인하므로 보관할 키나 비밀번호가 없습니다. 양쪽 HerdRabbit 버전이 같아야 하며, 다르면 그 서버가 양쪽 버전과 함께 offline으로 표시됩니다. 링크된 서버의 파일 탐색과 프로젝트 생성은 아직 지원하지 않습니다.

**SSH**는 Herdr는 있지만 HerdRabbit이 없는 머신용입니다. 이 경우 이 머신이 그 서버의 SSH 자격증명을 쥐고 SSH와 SFTP로 닿습니다.
...
SSH 연결은 유휴 상태에서 최대 60초간 재사용합니다. 원격 서버별 조회를 독립적으로 실행하고 연결 실패 시 재시도 간격을 늘립니다. 서버가 끊기면 마지막으로 확인한 패인 목록을 유지하고 오프라인으로 표시합니다. 원격 상태 구독은 별도 SSH Unix 소켓 포워딩을 사용하므로 SSH 서버에서 포워딩을 허용해야 합니다. 허용되지 않으면 HTTP 목록 조회로 상태를 갱신합니다. 터미널 출력은 같은 서버 감지/WebSocket delta 정책을 따르며, 서버가 여러 개이면 알림에도 서버명이 포함됩니다.
...
SSH 서버를 등록해 두면 Files 탭에 서버 선택이 생깁니다. 서버를 고르면 그 서버의 홈에서 트리가 시작되고, 서버마다 마지막으로 보던 폴더를 따로 기억합니다. 업로드는 보고 있는 세션을 따라갑니다 — 원격 세션을 선택한 상태에서 파일을 첨부하거나 붙여넣으면 **그 서버에** 저장되고, 입력창에 삽입되는 경로도 그 세션이 열 수 있는 경로입니다.

원격 탐색도 로컬과 마찬가지로 읽기 전용이며, 원격 업로드는 그 계정 홈의 `~/.local/share/herdrabbit/files`로 갑니다. 파일 전송이 그대로 따를 수 없는 SSH 설정(ProxyJump, PKCS#11·FIDO 키, GSSAPI, 암호가 걸린 개인키)을 쓰는 서버는 터미널과 다른 규칙으로 접속하는 대신 이유를 알려줍니다.
...
| `HERDR_WEB_ROLE` | `hub` | `hub`는 사람을 상대하고, `leaf`는 자기 허브만 상대하며 그 밖에는 아무것도 응답하지 않습니다. leaf는 loopback에 바인딩해야 하고, 피어를 하나 이상 지정해야 하며, 비밀번호가 없어야 합니다 — 아니면 프로세스가 기동을 거부합니다. |
...
| `HERDR_WEB_PEER_ADDRESSES` | 없음 | leaf가 받아들일 tailnet 주소(쉼표 구분). 로그인은 계정을 증명하지 기기를 증명하지 않으므로, 이것이 없으면 leaf가 그 계정의 모든 기기에 응답합니다. |
...
- 쓰기는 머신마다 하나씩 있는 업로드 폴더에만 닿습니다. 로컬에서는 비밀번호 해시·SSH 비밀번호·VAPID 키가 있는 설정 디렉터리 바깥에 두고, 원격 서버에서는 그 계정 홈의 `~/.local/share/herdrabbit/files`입니다. 어느 쪽이든 업로드와 삭제는 그 폴더 밖의 경로를 지정할 수 없습니다. URL로 들어온 이름은 교정하지 않고 거부하며, 대상은 경로를 조합하지 않고 디렉터리 목록에서 가져오고, 원격 업로드는 배타적 열기를 써서 기존 파일을 절대 덮어쓰지 않습니다.
- 탐색은 읽기 전용이며 **이 머신에서 서비스 계정이 읽을 수 있는 모든 경로, 그리고 등록된 각 SSH 프로필의 계정이 그 서버에서 읽을 수 있는 모든 경로**에 닿습니다. 경로 허용 목록을 두지 않고 OS 파일 권한을 경계로 삼습니다. 여기에는 그 계정들의 `~/.ssh`, 이 머신의 `~/.config/herdr-bridge/` 파일들, `/proc/self/environ`이 포함됩니다. 인증된 클라이언트는 이미 터미널 패인으로 그 계정들의 임의 명령을 실행할 수 있으므로 권한 상승은 아니지만, HerdRabbit을 root나 공용 계정으로 실행하지 말아야 할 이유가 하나 더 늘어납니다.
- **leaf**는 비밀번호가 아니라 Tailscale Serve가 붙이는 신원 헤더로 보호됩니다. 이는 Serve만이 그 소켓에 닿을 수 있는 동안에만 성립하므로, leaf는 loopback에 바인딩하지 않으면 기동을 거부하고 다른 경로로 온 요청도 거부합니다. 짚어둘 결과: **leaf의 어떤 로컬 프로세스든, 어떤 사용자로든 그 헤더를 위조해 전체 API에 닿을 수 있습니다.** 서비스 계정 본인에게는 달라지는 게 없지만(이미 `herdr` CLI가 있습니다), 그 머신의 다른 사용자에게는 비밀번호가 있던 경우보다 실질적으로 넓어집니다.
- leaf의 신원 검사는 **계정**을 확인하지 기기를 확인하지 않습니다. 그 계정이 가진 모든 기기가 leaf에 직접 닿는 것을 막는 것이 `HERDR_WEB_PEER_ADDRESSES`입니다.
- 링크는 여전히 SSH로 닿는 서버의 키 집중 위험을 없애지 않습니다. leaf로 전환한 머신에 대해서만 없애며, 그 머신의 키는 그 머신에 남습니다.
- 원격 탐색이 원격 계정의 읽기 권한을 그대로 쓰므로, **등록한 각 서버의 `~/.ssh`에 있는 개인키가 HerdRabbit에 침입한 사람의 손이 닿는 범위에 들어옵니다**. 세션 하나가 뚫리면 등록한 모든 서버의 키 자재가 함께 노출됩니다. 터미널을 통한 복사와 달리 SFTP 읽기는 어떤 패인 기록에도 남지 않으며, `Subsystem sftp -l INFO`를 켠 서버가 아니면 서버 쪽 로그에도 남지 않습니다.
- 파일 전송은 SSH를 직접 사용하므로 원격 호스트 키 검증을 OpenSSH가 아니라 HerdRabbit이 직접 합니다. 유효 설정이 지정하는 모든 `known_hosts` 파일을 `ssh-keygen -F`로 조회하며, `@revoked` 항목이 항상 우선하고, `@cert-authority` 줄은 호스트 키로 취급하지 않으며, 모르는 호스트는 거부합니다. 파일 전송이 그대로 따를 수 없는 설정(ProxyJump·ProxyCommand, PKCS#11·FIDO 키, GSSAPI, 암호가 걸린 개인키)을 쓰는 서버는 터미널과 다른 규칙으로 접속하는 대신 거부합니다.
