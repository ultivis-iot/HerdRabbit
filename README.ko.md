...
- 확인 후 세션 또는 프로젝트 종료
- 프로젝트와 세션 이름 변경
- 연결한 머신을 사이드바에서 관리: 이름 변경, 주소와 버전 확인, 포트를 옮긴 머신 다시 찾기, 연결 해제
...
서버는 전부 또 다른 HerdRabbit입니다. 다이얼로그가 tailnet의 머신 목록을 가져와 각각이 무엇인지 알려줍니다 — 추가 가능, 이미 추가됨, HerdRabbit은 있지만 이 머신을 안 받아줌, 버전 불일치. 추가 가능한 것을 고르면 이름과 주소가 채워집니다. 손댈 게 없는 머신(HerdRabbit 없음, 오프라인)은 개수만 남기고 접힙니다 — tailnet이 커도 목록이 읽히고, 기대했던 머신이 안 보일 때 펼쳐서 이유를 확인할 수 있습니다. 머신은 고르는 것이지 입력하는 것이 아닙니다. 주소는 tailnet에서 오고, 정할 것은 여기서 뭐라고 부를지뿐입니다.

기본 포트만 훑어서는 HerdRabbit을 여러 개 돌리는 머신을 찾을 수 없습니다 — 계정마다 하나씩 두면 tailnet 주소는 같고 포트만 다릅니다. 그런 머신은 스스로 알립니다. leaf는 어느 허브에 답할지 알고 설치되므로 그 허브에 자기 위치를 보내고, 허브는 그것을 탐색 결과와 함께 보여줍니다. 공지는 목록에 줄 하나를 넣을 뿐이고, 추가는 여전히 사람이 고르며 고를 때 그 주소를 직접 확인합니다. 머신은 자기 주소만 알릴 수 있습니다.

한 번 알린 내용은 만료시키지 않고 디스크에 남깁니다(`~/.config/herdr-bridge/announced.json`). 허브가 재시작했거나 leaf가 한동안 조용했다고 해서 사실이 아니게 되는 게 아니고, 정말 사라진 머신은 알아서 목록에서 빠집니다 — 후보는 제안되기 전에 항상 확인되기 때문입니다. 개수 상한이 있고, 가득 차면 가장 오래 아무 말이 없던 머신이 자리를 내줍니다. 확인해봤는데 없는 주소 — 머신은 응답하는데 그 포트에 아무것도 없는 경우 — 는 지웁니다. 그래야 포트를 옮긴 머신이 옛 항목을 영영 남기지 않습니다. 그냥 꺼져 있는 머신은 건드리지 않습니다. 다시 켜지면 스스로 알립니다.

이미 추가한 서버는 추가할 때의 주소를 계속 가리키므로, 포트를 옮긴 머신은 offline이 된 채로 남습니다. tailnet 주소가 둘이 같은 머신임을 말해주므로, 연결되지 않는 서버에는 사이드바에 **Find new address**가 생깁니다. 다시 찾아보고 그 머신이 다른 포트에서 답하면 주소만 바꾸며 붙여둔 이름은 유지합니다.

연결할 게 없는 머신은 개수만 세고 나열하지 않습니다. 다만 HerdRabbit이 돌고 있는데 이 허브를 거절하는 머신은 이유와 함께 나열합니다 — 그 머신은 다른 허브를 보고 있어 여기에 공지하지 않으므로, 확인해봐야만 알 수 있습니다.

상대 머신을 준비하려면 거기에 **leaf 모드**로 설치하면서 이 머신을 허브로 지정하면 됩니다 — 다이얼로그가 이 머신의 tailnet 주소를 함께 보여줍니다. 이쪽에 저장되는 것은 이름과 주소뿐입니다 — leaf는 요청이 도착한 주소로 자기 허브를 알아보므로 어디에도 보관할 키나 비밀번호가 없습니다. **Save**는 지금 폼의 주소로 **Test connection**이 성공해야 열리고, 주소를 고치면 다시 잠깁니다.
...
허브와 leaf는 같은 코드, 같은 설치입니다. 서비스 유닛만 다릅니다. 역할을 바꾸려면 설치 스크립트를 다시 돌리세요 — 질문의 기본값이 현재 상태라, 그냥 엔터만 눌러 넘기면 아무것도 바뀌지 않습니다. 허브를 leaf로 바꾸면 그 머신의 비밀번호와 등록된 passkey가 삭제되고(leaf에는 로그인이 없습니다) Tailscale Serve 등록도 해제됩니다. 되돌리면 비밀번호를 다시 묻고 Serve에 다시 등록합니다.
...
서버는 보이는 자리에서 관리합니다. 사이드바의 `⋯` 메뉴에서 이름을 바꾸고, 연결을 끊고, **Details**로 주소와 거기서 답하는 HerdRabbit 버전, 그 머신이 내주는 Herdr 세션과 패인 수를 봅니다. 머신이 연결되지 않을 때 가장 먼저 확인할 값들입니다. 머신을 추가하는 다이얼로그는 추가만 합니다.
...
- 세션 오른쪽 `⋯` 메뉴의 **이름 변경**도 같은 방식으로 그 자리에서 편집합니다.
...
세션은 이름을 주기 전까지 그 안에서 돌고 있는 에이전트를 보여줍니다. 그건 이름이 아닙니다 — 에이전트가 바뀌면 같이 바뀌고, 같은 에이전트를 쓰는 두 세션이 똑같이 읽힙니다. 이름을 주면 에이전트가 있던 자리를 이름이 가져가고 에이전트는 상태 옆으로 물러납니다. 패인이 여럿인 세션은 버튼마다 반복하지 않고 이름을 묶음 위에 둡니다.

상태 글자(`idle`, `working`)는 세션 옆에 적지 않습니다. 옆의 표식이 이미 상태를 말하고, done이나 blocked인 세션은 테두리와 광채로 한 번 더 말합니다. 정확한 상태가 필요하면 세션에 마우스를 올리거나 스크린리더로 읽으면 글자로 나옵니다.

이름을 비우면 다시 떼어낼 수 있고, 세션은 에이전트를 다시 보여줍니다. Herdr에는 이름 없는 탭이 없어서 — `tab rename`은 라벨을 요구합니다 — 지우기는 탭이 처음 받은 번호를 되돌려 씁니다. 사이드바가 숨기는 값입니다.

프로젝트 이름 변경은 Herdr의 `workspace rename`, 세션 이름 변경은 `tab rename` 명령에 전달됩니다. 둘 다 그 머신 앞에 앉은 사람이 실행하는 것과 같은 명령입니다.

### 프로젝트와 세션 생성·종료

- 사이드바 상단의 `+` 버튼은 이름을 지정한 새 프로젝트와 기본 셸을 만듭니다.
- 프로젝트명 오른쪽 `⋯` 메뉴의 **새 세션**은 그 프로젝트에 기본 셸 세션을 하나 추가합니다.
- 세션 오른쪽 `⋯` 메뉴의 **이름 변경**은 그 세션의 이름을, **세션 종료**는 해당 Herdr 탭을 종료합니다.
...
머신이 둘 이상 등록되면 Files 탭에 서버 선택이 생깁니다. 고른 머신의 홈에서 트리가 시작되고, 머신마다 마지막으로 보던 폴더를 따로 기억합니다.

그 선택이 업로드가 가는 곳이기도 합니다. 고르기 전에는 보고 있는 세션을 따라가고 경로가 입력창에 삽입됩니다. 머신을 고르면 탐색과 업로드 양쪽에 고정되며 새로고침해도 유지됩니다. 고정한 머신이 지금 세션이 도는 머신과 다르면 경로를 **삽입하지 않습니다** — 그 세션이 열 수 없는 파일을 가리키게 되니까요 — 대신 어디에 올라갔는지 알려줍니다. 첨부 버튼과 업로드 다이얼로그는 어느 쪽이든 대상 머신을 표시합니다.
...
출력과 이전 기록은 Herdr의 ANSI 화면 및 스크롤백에서만 가져오며 Claude JSONL 로그는 조회하지 않습니다. 상단으로 스크롤하면 이전 기록을 200줄씩, 최대 100,000줄까지 요청합니다. 설치·업데이트 시 Claude 전역 설정에 `tui: "default"`와 `env.CLAUDE_CODE_DISABLE_ALTERNATE_SCREEN: "1"`을 적용하고 기존 설정은 백업합니다. 실행 중인 Claude는 재시작 후 적용됩니다. 머신마다 자기 HerdRabbit을 돌리므로, 각 머신의 설치·업데이트에서 그 머신에 적용됩니다.
...
| `HERDR_WEB_HUB` | 없음 | leaf가 자기를 알릴 허브. `https://hub.tailnet.ts.net:38787` 같은 전체 URL입니다. 주소가 아니라 이름인 이유는 허브가 Tailscale Serve 뒤에 있고 인증서가 tailnet 이름으로 발급되기 때문입니다. 비워 두면 leaf는 발견되기를 기다리며, 그건 기본 포트에서만 동작합니다. |
