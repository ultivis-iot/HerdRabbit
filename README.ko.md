...
무엇을 무엇으로 보여줄지는 바이트를 훑어서가 아니라 이름을 허용목록에 맞춰 정하며, 실행될 수 없는 형식만 그 목록에 있습니다. SVG와 HTML은 의도적으로 빠져 있고, 대신 자기를 이루는 글자 그대로 읽힙니다 — 텍스트는 다운로드와 똑같은 불투명한 바이트로 받아서 페이지에 글자로 써넣으므로 서버에 타입을 묻는 일 자체가 없습니다. 다운로드는 그대로입니다. 여전히 `application/octet-stream` 첨부이고, 그것이 저장된 페이지가 이 출처에서 실행되지 못하게 막는 장치입니다.
