...
SSH 비밀번호는 서비스 메모리에만 유지하고 파일이나 프로필 API 응답에는 포함하지 않습니다. 앱을 다시 열어도 유지되지만 HerdRabbit 서비스가 재시작되면 **Connect SSH Server → Edit**에서 다시 입력하고 저장해야 합니다. 비밀번호 연결을 수정할 때도 다시 입력합니다. 자격 증명 입력에는 Tailscale Serve 같은 HTTPS 연결을 사용하세요. 일반 HTTP는 전송 내용을 암호화하지 않습니다.

프로필은 인증 파일과 같은 디렉터리의 `ssh-profiles.json`에 `0600` 권한으로 저장합니다. 기본 경로는 `~/.config/herdr-bridge/ssh-profiles.json`이며 `HERDR_WEB_SSH_PROFILES_FILE`로 변경할 수 있습니다. 개인키 내용과 SSH 비밀번호는 프로필에 저장하지 않습니다. 같은 HerdRabbit 인스턴스를 사용하는 기기들은 이 프로필을 공유합니다. 원격 계정을 조작할 수 있으므로 인스턴스의 비밀번호/Passkey 로그인을 설정해서 사용하세요.
