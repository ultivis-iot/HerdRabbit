...
- 목록 끝의 **Add account**는 `AI login: …` 프로젝트를 열고 그곳에 로그인 명령을 입력합니다. `claude auth login` 또는 `codex login --device-auth`이며, 각자 비어 있는 전용 디렉터리를 가리킵니다. 지금 쓰는 계정이 있는 곳에서 로그인하면 그 계정의 토큰이 폐기되기 때문에 따로 떨어진 곳에서 로그인합니다. Codex의 device code 로그인은 ChatGPT 보안 설정에서 먼저 켜야 합니다. 터미널에 로그인됐다고 나오면 AI accounts를 다시 열고 **Finish**를 누르세요. 프로젝트는 닫힙니다. 그 프로젝트에 입력한 내용은 입력 기록에 남지 않습니다.
- **Switch**는 머신 전체의 계정을 바꿉니다. 설정, MCP 서버, 대화 기록, 메모리는 그대로라서 `claude --continue`나 `codex resume`으로 다른 계정이 멈춘 곳부터 이어갈 수 있습니다. 바뀌기 전 계정은 먼저 저장하므로, 쓰는 동안 CLI가 갱신한 토큰도 보존됩니다. Herdr가 어느 패인에서든 그 CLI가 실행 중이라고 알려주면 해당 패인 목록을 보여주고 확인을 받습니다. 그 패인들은 재시작하기 전까지 이전 계정을 씁니다. Herdr 밖에서 실행한 CLI는 감지하지 못합니다.
- 계정 줄 ⋯ 메뉴의 **Remove**는 저장된 사본을 지웁니다. 어디에서도 계정을 로그아웃시키지는 않습니다.
