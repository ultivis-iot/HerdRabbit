...
무엇을 무엇으로 보여줄지는 바이트를 훑어서가 아니라 이름을 허용목록에 맞춰 정합니다. 아무것도 실행되지 않되, 실행될 수 있는 두 형식은 거절하는 대신 보여주는 방식으로 다룹니다. SVG는 스크립트를 실행하지 않는 `<img>`에 넣고, HTML은 바이트로 받아 `sandbox=""` iframe에 넣습니다 — 스크립트도 없고 출처도 불투명합니다. 둘 다 집으로 전화를 거는 파일로 실측했고 아무것도 빠져나가지 못했습니다. 원본 URL을 직접 열어도 앱 자신의 `script-src 'self'`가 인라인 스크립트를 막습니다. 어느 쪽이든 버튼 한 번으로 소스로 볼 수 있습니다. 텍스트도 같은 방식입니다 — 불투명한 바이트로 받아 글자로 써넣고, 타입을 협상하지 않습니다. 다운로드는 그대로입니다. 여전히 `application/octet-stream` 첨부이고, 그것이 저장된 페이지가 이 출처에서 실행되지 못하게 막는 장치입니다.
