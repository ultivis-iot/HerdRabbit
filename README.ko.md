...
이미 추가한 서버는 추가할 때의 주소를 계속 가리키므로, 포트를 옮긴 머신은 offline이 된 채로 남습니다. tailnet 주소가 둘이 같은 머신임을 말해주므로, 연결되지 않는 서버 옆에 그 머신이 준비된 다른 포트가 있으면 따라가겠냐고 제안합니다. 한 번 누르면 주소만 바뀌고 붙여둔 이름은 유지됩니다.
